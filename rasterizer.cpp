...
void rasterizer::setModelMat(Eigen::Matrix4f mat) {
    this->modelMat = mat;
}
void rasterizer::setViewMat(Eigen::Matrix4f mat) {
    this->viewMat = mat;
}
void rasterizer::setProjMat(Eigen::Matrix4f mat) {
    this->projMat = mat;
}
...
Eigen::Vector3f rasterizer::getFrameBuf(int i) {
    return frame_buf[i];
}
...
static std::tuple<float, float, float> computeBarycentric2D(float x, float y, std::array<Eigen::Vector4f, 3ULL> &v) {
...
void rasterizer::render(std::vector<Triangle *> triangleList) {
    ...
    for (int i = 0; i < triangleList.size(); i++) {
        ...
        Triangle newTri = *t;
        Eigen::Vector4f v[] = {mvpMat * t->vert(0), mvpMat * t->vert(1), mvpMat * t->vert(2)};

        std::array<Eigen::Vector4f, 3> viewSpaceVertexs4 = {
            (viewMat * modelMat) * t->vert(0), (viewMat * modelMat) * t->vert(1), (viewMat * modelMat) * t->vert(2)};

        std::array<Eigen::Vector3f, 3> viewSpaceVertexs3;

        std::transform(viewSpaceVertexs4.begin(), viewSpaceVertexs4.end(), viewSpaceVertexs3.begin(), [](auto &v) {
...
void rasterizer::drawTriangle(const Triangle &t, std::array<Eigen::Vector3f, 3> &viewSpaceVertexs) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        maxPix[0] = std::max(maxPix[0], v[i].x());
        maxPix[1] = std::max(maxPix[1], v[i].y());
    }
        ...
        for (int x = int(minPix[0]); x <= int(maxPix[0]); x++) {
            ...
            if (insideTriangle(cx, cy, v)) {
                ...
                if (depth_buf[ind] > zp) {
                    ...
                    Eigen::Vector3f interpolated_normals =
                        interpolate(alpha, beta, gamma, t.norm(0), t.norm(1), t.norm(2), 1);
